...
const filterBooksByGenre = async () => {
    ...
    const checkedGenres = document.querySelectorAll('input[name=genre-checkbox]:checked');
...
const loginOrRegister = async action => {
    const url = action === 'login' ? '/api/korisnik/uloguj' : '/api/korisnik/registruj';
...
const addEventListenersToCheckboxes = async () => {
    ...
    checkboxes.forEach(checkbox => checkbox.addEventListener('click', filterBooksByGenre));
};
